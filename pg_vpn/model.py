...
def rollout_step(x, a, n_actions, op_trans_state, op_trans_pred,
    ...
    if a is not None:
        ...
        a = tf.one_hot(a_idx, n_actions, dtype=tf.int32) # [batch_size, top_k, n_action]
...
class Model(object):
    def __init__(self, ob_space, n_actions, type, 
        ...
        if self.type == 'policy':
            ...
            self.vf = tf.reshape(self.op_value(self.s), [-1])
            ...
            for i in range(depth):
                ...
                idx = tf.to_int32(tf.gather_nd(a_list[i], idx_idx)) # re-order

                idx_list.append(idx)

                l = tf.tile(tf.expand_dims(tf.range(0, tf.shape(idx_b)[0]), 1), [1, b])
                l = tf.concat([tf.reshape(l, [-1, 1]), tf.reshape(idx_b, [-1, 1])], axis=1)
    ...
    def act(self, ob, state_in=[], meta=None):
        ...
        if self.type == 'policy':
            ...
            out = sess.run([self.q_plan, self.vf] + self.state_out, feed_dict)
            q = out[0]
            v = out[1]
            state_out = out[2:]
            act = np.zeros_like(q)
            ...
            # return sess.run([self.sample, self.vf] + self.state_out, feed_dict) # testing
    ...
    def value(self, ob, state_in=[], meta=None):
        ...
        if self.type == 'policy':
            ...
            q = sess.run(self.q_plan, feed_dict)
            return q.max()
            # return sess.run(self.vf, feed_dict)[0] # testing
